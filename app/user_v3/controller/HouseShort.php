...
use app\user_v3\model\HouseImg;
use app\user_v3\model\HouseTag;
use app\user_v3\model\RoomConfig;
use app\user_v3\model\ShortComment;
use app\user_v3\model\ShortOrder;
use app\user_v3\model\ShortTrafficTag;
...
class HouseShort extends Base
{
     ...
     * 短租房源列表
     */
    public function houseList(HouseShortModel $HouseShortModel){
...
OR `ruzhu_end_time` BETWEEN '{$start_time}' AND '{$end_time}')";
        ...
        $total = $HouseShortModel->where($house_where)->count();


        $list = HouseShortModel::getHouseShortList($page,$size,$house_where);
    ...
    public function houseDetail(){
        ...
        $data = HouseShortModel::getHouseShortDetail($param['short_id']);
    ...
    public function commentList(){
        ...
        $house_short = HouseShortModel::get($short_id);
        if (!$house_short){
...
OR `ruzhu_end_time` BETWEEN '{$param['start_time']}' AND '{$param['ruzhu_end_time']}')";
     ...
     * 获取支付信息
     */
    public function getPayInfo(ShortOrder $orderModel){
        try{
            //token 验证
            $userInfo = UserFunc::checkToken();
            if ($userInfo instanceof Json){
    ...
    public function orderDetail(){
        ...
        $house_info = HouseShortModel::with('houseShortImg')->field('id,title,rent,bedroom_number,parlour_number,toilet_number,acreage')->where('id',$order['short_id'])->find();
     ...
     * 评论短租
     */
    public function comment(ShortComment $ShortComment){
        ...
        try{
            ...
            $result1 = $ShortComment->allowField(true)->save($param);
