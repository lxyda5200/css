    ...
    public function autoCancelNotPayOrder(){
        ...
        try{
            ...
            if(is_array($data)){
                ...
                addErrLog($e->getMessage(),'30分钟未支付订单自动取消',4);
            }
...
//            if($res === false)throw new Exception("\n\r" . "待发货自动取消订单返还优惠券[店铺]失败=>订单IDS【". json_encode($order_ids) ."}】,优惠券IDS=>【"  . json_encode($store_coupon_ids) ."】");
            ...
            foreach($list as $v){
                ...
                if ($res !== true)throw new Exception(json_encode(['content'=>['订单IDS'=>$order_ids,'退款类型'=>$v['pay_type'],'当前单号'=>$order_no],'title'=>"待发货自动取消订单退款失败"]));
            }
            ...
            if(is_array($data)){
                ...
                addErrLog($e->getMessage(),'待发货7天未发货自动取消订单',4);
            }
    ...
    public function autoAgreeAfterSale(){
        ...
        try{
            ...
            foreach($refund_lists as $k => $v){
                ...
                if ($res !== true)addErrLog(['售后IDS'=>$shouhou_ids,'订单号'=>$k,"退款类型"=>$v['pay_type'],'错误信息'=>$res],'售后自动同意退款失败',3);
            }
            ...
            if(is_array($data)){
                ...
                addErrLog($e->getMessage(),'申请售后商家7天未处理自动同意',4);
            }
    ...
    public function autoShouhouArrivePro(){
        ...
        try{
            ...
            foreach($refund_lists as $k => $v){
                ...
                if ($res !== true)addErrLog(['售后IDS'=>$shouhou_ids,'订单号'=>$k,"退款类型"=>$v['pay_type'],'错误信息'=>$res],'售后自动确认商家收货【退款失败】',3);
            }
            ...
            if(is_array($data)){
                ...
                addErrLog($e->getMessage(),'售后=》用户退货15天后商户未确认收货退款【自动确认收货并退款】',4);
            }
