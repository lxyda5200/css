    ...
    public static function productActivityInfo($product_id){
        ...
        switch((int)$activity_info['activity_type']){
            ...
            case 4:  ##打折
                ...
                $rule[] = "全场商品{$discount}折，最高可减{$activity_info2['discount_max']}";
                break;
            case 5:  ##返现
                $type = "返现";
...
//        }
    ...
    public static function getAcNewPersonInfo($activity_id, $user_id){
        ...
        return returnArr(1,'',compact('banners','recom_coupons','activity'));
    }
    ...
    public static function getActivityInfo($activity_id){
        return Db::name('activity')->where(['id'=>$activity_id])->field('id,banner_ids,recom_coupon_ids,activity_type,activity_pro_type,start_time,end_time,is_limit_time,status')->find();
    }
    ...
    public static function autoAcStartTime($activity_info){
        ##判断活动状态
        if($activity_info['start_time'] > time()){  ##活动尚未开始
            $status = 1; //活动未开始
            if($activity_info['activity_type'] != 2 && $activity_info['activity_type'] != 4){  ##不可限时的活动
                ...
                if($activity_info['is_limit_time']){  ##限时活动
                    ...
                    ###查找有无开始日的限时
                    $limit_time_today = Db::name('activity_limit_time')->where(['activity_id'=>$activity_info['id'],'week_day'=>$week,'start_time'=>['GT',$time]])->field('week_day,start_time,end_time')->find();
                    if($limit_time_today){
                        $start_time = date('Y-m-d',$time_start) . " " . $time;
        ...
        }else if($activity_info['start_time'] <= time() && $activity_info['end_time'] > time()){  //活动进行中
            $status = 2;
            if($activity_info['activity_type'] != 2 && $activity_info['activity_type'] != 4){  ##不可限时的活动
                $start_time = $activity_info['start_time'];
                $end_time = $activity_info['end_time'];
    ...
    public static function getAcModernInfo($activity_id){
        ...
        return returnArr(1,'',compact('banners','activity'));
    ...
    public static function getAcGreatBrandInfo($activity_id){
        ...
        return returnArr(1,'',compact('list','activity','rule'));
