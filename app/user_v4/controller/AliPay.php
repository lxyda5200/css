...
//                Db::name('member_order')->where('order_no',$out_trade_no)->update(['pay_type'=>'苹果内购','pay_time'=>time(),'status'=>2]);
    ...
    public function maidan_alipay_notify(){
        ...
        if ($verify_result) {//验证成功
            ...
            if($trade_status == 'TRADE_FINISHED' || $trade_status == 'TRADE_SUCCESS') {
                #获取订单信息
                $order_info = Db::name('maidan_order')->where(['order_sn'=>$out_trade_no])->field('id,store_id,status,user_id,price_maidan,price_yj,member_order_id,coupon_id,coupon_money,price_store')->find();
                if(!$order_info){
                    addErrLog($_POST,"买单支付回调订单号{$out_trade_no}不存在",6);
                    Log::write("订单号{$out_trade_no}不存在");
                    Log::write('---End支付宝回调---');
                    exit();
                }
                if($order_info['status'] != 1){
                    addErrLog($_POST,"买单支付回调=>订单号{$out_trade_no} => 订单状态异常不存在,当前状态【{$order_info['status']}】",6);
                ...
                if($res === false){
                    addErrLog($_POST,"买单支付回调=>订单号{$out_trade_no} => 订单信息更新失败",6);
