...
use app\user_v4\common\Logic;
use app\user_v4\common\User as UserFunc;
use app\user_v4\common\UserLogic;
use app\user_v4\model\Giftpack;
use app\user_v4\model\GiftpackOrder;
use app\user_v4\model\MemberOrder;
use app\user_v4\model\Product as Pmodel;
use app\user_v4\model\ProductComment;
use app\user_v4\validate\ProductValidate;
use think\Cache;
use think\Db;
use app\user_v4\common\User;
use think\Exception;
use think\Log;
use think\response\Json;
use app\user_v4\validate\CouponValidate;
use think\Config;
    ...
    public function storeDetail(){
        ...
        try {
            ...
            $coupons = Db::name('coupon_rule')
                ...
                ->where(['client_type'=>['IN',[0,2]],'use_type'=>['IN',[1,3]],'coupon_type'=>['IN',[3,4]]])  //卡券的适用平台限制
...
//            $data = Cache::get('product_'.$product_id.$product_specs);
            ...
            if($activity_info['status'] != 3){
                $activity[0]['type'] = $activity_info['activity_pro_type'] == 4?1:2;
...
//            $data['link'] = request()->domain() . "/index.php/user/index/product_content_p/id/{$data['id']}.html";
            $data['link'] = "http://121.196.214.146/css/public/index.php/user_v4/index/product_content_p/id/{$data['id']}.html";
            $data['product_img'] = Db::name('product_img')->where('product_id',$product_id)->column('img_url');
        ...
        try{
            ...
            if($card_info){
                    ...
                    if($order_id){
                        switch($pay_type){
                            case 1:
                                $notify_url = SERVICE_FX."/user_v4/ali_pay/member_alipay_notify";
                                $aliPay = new AliPay();
                                $data['alipay_order_info'] = $aliPay->appPay($order_no,$pay_money,$notify_url);
                                break;
                            case 2:
                                $notify_url = SERVICE_FX."/user_v4/wx_pay/member_wxpay_notify";
                                $wxPay = new WxPay();
...
//            $ids = "3,4,5,2,1,9043,9044,9602";
    ...
    public function getActivityInfo(){
        ...
        try{
            ...
            return \json(self::callback(1,'',$data['data']));
    ...
    public function getAcModernPro(){
        try{
            ...
            foreach($list as &$v)foreach($v['product'] as &$vv)$vv['activity_status'] = $activity_status;
