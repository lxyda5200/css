...
use app\user_v4\common\Orders;
use app\user_v4\common\UserLogic;
use app\user_v4\model\ProductOrder;
use app\user_v4\model\ProductShouhou as productshouhouModel;
use think\Db;
...
//                    'coupon_money' => round(($coupon_money/$store_number)),
            ...
            switch($pay_type){
                case 1:
                    $pay_type = "支付宝";
                    $notify_url = SERVICE_FX."/user_v4/ali_pay/goods_alipay_notify";
                    $aliPay = new AliPay();
                    $data['alipay_order_info'] = $aliPay->appPay($pay_order_no,$pay_money,$notify_url);
                    break;
                case 2:
                    $pay_type = "微信";
                    $notify_url = SERVICE_FX."/user_v4/wx_pay/goods_wxpay_notify";
                    $wxPay = new WxPay();
    ...
    public function Pay(){
        try{
            ...
            switch($pay_type){
                case 1:
                    $pay_type = "支付宝";
                    $notify_url = SERVICE_FX."/user_v4/ali_pay/repay_alipay_notify";
                    $aliPay = new AliPay();
                ...
                case 2:
                    $pay_type = "微信";
                    $notify_url = SERVICE_FX."/user_v4/wx_pay/repay_wxpay_notify";
                    $wxPay = new WxPay();
...
//            ];
            ...
            $num2=0;
            foreach ($num as $k=>$v){
                if(($v['is_comment']==1 && $v['status']==0 && $v['is_shouhou']==0) || ($v['is_comment']==0 && $v['status']!=0 && $v['is_shouhou']==0) || ($v['is_comment']==0 && $v['status']==0 && $v['is_shouhou']!=0)){
                    $num2+=1;
                }
            }
            //查询评论数量
...
//------------------
                ...
                $list =  Db::view('product_order','id as order_id,order_no,pay_order_no,store_id,pay_money,total_freight,order_status,address_status,pt_id,order_type,distribution_mode as express_mode,pt_type')
                    ->view('store','store_name,cover,type,type as store_type','store.id = product_order.store_id','left')
...
//            $goods_status = input('goods_status');
           ...
           // $refund_times = input('refund_times') ? intval(input('refund_times')) : 1 ;//次数
            ...
            if ($order->order_status != 5 && $order->order_status != 6 && $order->order_status != 7) {
    ...
    public function refundDetail(){
        try {
            ...
            $order = $orderModel->where('id',$order_id)->find();
            if (!$order){throw new \Exception('该订单不存在');}
            if ( $order->order_status != 5 && $order->order_status != 6 && $order->order_status != 7) {throw new \Exception('该订单不支持此操作');}
...
//                ->where('coupon_rule.end_time','gt',$time2)
    ...
    public function maidanOrder(UserValidate $UserValidate){
        ...
        try{
            ...
            switch($pay_type){
                case 'wx':
                    $notify_url = SERVICE_FX."/user_v4/wx_pay/maidan_wxpay_notify";
                    $wxPay = new WxPay();
                    $data['wxpay_order_info'] = $wxPay->getOrderSign($order_sn,$price_maidan,$notify_url);
                    break;
                case 'zfb':
                    $notify_url = SERVICE_FX."/user_v4/ali_pay/maidan_alipay_notify";
                    $aliPay = new AliPay();
...
//            if($price_maidan < 0.01) return \json(self::callback(0,'支付金额不能小于0.01元'));
            ...
            $discount_platform = $discount_type == 1?10:$maidan_info['member_user'];
        ...
        ##计算平台优惠金额
        $platform_policy = $price_yj_2 - $price_maidan - $store_policy;
