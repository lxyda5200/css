...
use app\user_v4\common\Login;
use app\user_v4\model\HouseEntrust;
use app\user_v4\validate\Login as LoginValidate;
use app\user_v4\common\Login as LoginFunc;
...
use app\user_v4\validate\User as UserValidate;
use app\user_v4\validate\UserAddress as UserAddrValidate;
use app\user_v4\model\UserAddress as UserAddrModel;
use app\user_v4\model\House as HouseModel;
use app\user_v4\model\HouseShort as HouseShortModel;
use app\user_v4\model\Store as StoreModel;
...
//            $UserModel->allowField(true)->save();
    ...
    public  function login(UserValidate $validate, UserModel $UserModel){
        try{
            ...
            if($result === false){return \json(self::callback(0,'操作失败'));}
    ...
    public function forgetPassword(UserValidate $validate){
        try {
            ...
            if (!$param) {return json(self::callback(0,'参数错误'),400);}
            if (!$validate->check($param,[],'forgetPwd')) {return json(self::callback(0,$validate->getError()),400);}
            $verify = IhuyiSMS::verifyCode($param['mobile'],$param['code']);
            if (!$verify) {throw new \Exception('验证码不存在或已失效');}
            ##检测手机号是否注册
            $mobile = input('post.mobile','','addslashes,strip_tags,trim');
...
//            $userInfo['card_number'] = Db::name('user_card')->where('user_id',$userInfo['user_id'])->count();
            ...
            if($member){
                $userInfo['member_card_id'] = $member['member_card_id'];
                $userInfo['member_price'] = $member['price'];
            }
            $user_id = $userInfo['user_id'];
...
//        $list = $store_id > 0?(UserLogic::userOrderCouponLists($user_id,$store_id,$type,$page,$size)):(UserLogic::userCouponLists($user_id,$type,$page,$size));
    ...
    public function userGetCoupon(UserValidate $UserValidate){
        ...
        if(!$store_id && ($coupon_info['type'] == 2 || $coupon_info['type'] == 1))return \json(self::callback(0,'优惠券领取失败'));
...
//        }
            ...
            $list = Db::name('coupon_rule')->alias('cr')
                ...
                ->where($where)
                ->where($where1)
                ->field('cr.id,cr.coupon_money,cr.satisfy_money,cr.type,cr.coupon_type,cr.fb_user,cr.is_solo,cr.zengsong_number,s.cover,s.store_name,cr.is_superposition,cr.product_ids,cr.store_id,cr.coupon_name,cr.end_time')
                ->select();
            //如果领取过
            if($user_id){
 ...
 * */
    public function store_collection_and_cancel(){
        try{
            //token 验证
            $userInfo = \app\user_v4\common\User::checkToken();
            if ($userInfo instanceof Json){
                return $userInfo;
            }
            $param = $this->request->post();
    ...
    public function maidanCouponList(UserValidate $UserValidate){
        try{
            ...
            $list = Db::name('coupon')->alias('c')
                ...
                ->where($where)
                ->where(['cr.client_type'=>['IN',[0,2]],'cr.use_type'=>['IN',[1,3]]])  //卡券的适用平台限制
                ->field('c.id,cr.coupon_name,cr.satisfy_money,cr.can_stacked,cr.coupon_money,c.expiration_time,cr.coupon_type,cr.type,s.store_name,p.product_name,cr.is_superposition,cr.store_id,cr.coupon_name')
    ...
    public function userShareInfo(){
        try{
            ...
            #逻辑
            $code_img = "{$web_url}/app/download.png";
