...
class Product extends Model
{
    ...
    public function getRecomProductList(){
        ...
        if($store_id)$where['p.store_id'] = $store_id;
        if($product_ids[0])$where['p.id'] = ['NOT IN',$product_ids];
     ...
     */
    public function productSpecsList(){
        $product_ids = input('post.product_ids','','trimStr');
        $product_ids = explode(',',$product_ids);
        $list = $this->where(['id'=>['IN', $product_ids]])->field('id,product_name')->with(['specs'])->select()->toArray();
        foreach($list as &$v){
            foreach($v['specs'] as &$vv){
                $vv['specs'] = implode('-',array_values(json_decode($vv['product_specs'],true)));
            }
        }
        return $list;
    }
    ...
    public function specs(){
        return $this->hasMany('ProductSpecs','product_id','id')->field('id as specs_id,product_id,price,cover,product_specs')->order('price','asc');
    }
