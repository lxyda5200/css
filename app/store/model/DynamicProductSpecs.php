...
class DynamicProductSpecs extends Model
{
     ...
     */
    public static function getDynamicSpecs($dynamic_product_id, $dynamic_id){
        return (new self())->alias('dps')
            ->join('product_specs ps','dps.specs_id = ps.id','LEFT')
            ->where([
                'dps.dynamic_product_id'=>$dynamic_product_id,
                'dps.dynamic_id' => $dynamic_id
            ])
            ->field('
                dps.price,dps.specs_id,
