...
use app\store\model\OrderBusiness;
use app\store\model\ProductOrder;
...
use app\store\model\MaiDanOrder;
...
//            $logistics_info = input('logistics_info');  //物流信息
            ...
            ##添加订单操作记录
            OrderBusiness::addHandleLog(json_decode(json_encode($product_order),true));
            ...
            ##生成系统消息
            Msg::addDeliverGoodsSysMsg($order_id);
            ##第三方推送
            JiG::sendMsgToUser("u{$product_order->user_id}", 'order_send_pro', ['order_id'=>$order_id]);
            ...
            if($status=='true') {
                ...
                if (($type == 1 && $refund_type == 1) || ($type == 2 && $order_status['refund_type'] == 2)) {
                    ...
                    if ($order_product['realpay_money'] > 0) {
                        ...
                        if ($res) {
                            ##3退款通知
                            Msg::addShouhouRefundSysMsg($order_info['id']);
                            ##推送消息
                            JiG::sendMsgToUser("u{$order_info['user_id']}", 'shouhou_refund', ['order_no'=>$order_info['order_no']]);
                            ...
                            if ($rst1 === false || $rst2 === false) {
                                return \json(self::callback(0, '更新退款信息状态失败!'), 400);
                            }
                            //判断是否该订单全部退款/退货退款
...
//                                }
                ...
                ##添加系统消息
                Msg::addShouhouPassSysMsg($order_info['id']);
                ##发送推送消息
                JiG::sendMsgToUser("u{$order_info['user_id']}",'shouhou_pass', ['order_no'=>$order_info['order_no']]);
...
//                $num2 = Db::name('product_order_detail')->where('order_id',$order_product['order_id'])->where('is_shouhou','in','-1,-2,4')->count();
                ...
                ##生成系统消息
                Msg::addShouhouRefuseSysMsg($order_info['id']);
                ##发送推送消息
                JiG::sendMsgToUser("u{$order_info['user_id']}",'shouhou_refuse', ['order_no'=>$order_info['order_no']]);
