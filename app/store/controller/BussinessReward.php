...
use app\store\repository\implementses\RBussinessReward;
use app\store\validate\BussinessReward as BussinessRewardValidate;
...
class BussinessReward extends Base
{
    ...
    public function addReward() {
        ...
        if($params['condition'] == 0)
            return json(self::callback(0, '奖励条件不能为0'));
        if($params['reward'] == 0)
            return json(self::callback(0, '奖励金额不能为0'));
        # 获取之前奖励最大值
        $max_reward = RBussinessReward::getPreRewardNotId($params['store_id'], $params['condition']);
    ...
    public function editReward() {
        ...
        # 获取上一阶段奖励最大值
        $max_reward = RBussinessReward::getPreReward($params['store_id'], $params['condition'], $params['id'])? : 0;
        if($params['reward'] <= $max_reward)
            return json(self::callback(0, '奖励金额必须大于上一阶段金额'));
        # 获取下一阶段奖励最小值
        $next_reward = RBussinessReward::getNextReward($params['store_id'], $params['condition'], $params['id'])? : 0;
