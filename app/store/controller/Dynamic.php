...
use app\store\model\DynamicProduct;
use app\store\model\DynamicStyle;
use app\store\model\Scene;
use app\store\model\StoreStyleStore;
...
use app\store\validate\Dynamic as DynamicValidate;
use app\store\model\Dynamic as DynamicModel;
use app\store\model\DynamicImg;
...
class Dynamic extends Base
{
    ...
    public function addDynamic(DynamicValidate $dynamic, DynamicModel $dynamicModel, DynamicImg $dynamicImg, Scene $scene, DynamicStyle $dynamicStyle, DynamicProduct $dynamicProduct){
        ...
        try{
            ...
            if($res === false)throw new Exception('添加失败');
            ###增加标签
            $res = $dynamicProduct->add($dynamic_id, $post['products'], $img_ids, $post['is_group_buy']);
     ...
     * @return array|false|\PDOStatement|string|\think\Model|Json
     */
    public function dynamicImgList(DynamicValidate $dynamic, DynamicModel $dynamicModel){
        try{
            ##token 验证
            $store_info = Store::checkToken();
            if ($store_info instanceof Json){
            ...
            ##验证参数
            $res = $dynamic->scene('dynamic_img_list')->check(input());
            if(!$res)throw new Exception($dynamic->getError());
            ##逻辑
            $data = $dynamicModel->getDynamicImgList();
            ##返回
            return json(self::callback(1,'',$data));
        }catch(Exception $e){
            return json(self::callback(0,$e->getMessage()));
     ...
     * @return array|false|\PDOStatement|string|\think\Model|Json
     */
    public function editDynamicImg(DynamicValidate $dynamic, DynamicImg $dynamicImg){
        try{
            ##token 验证
            $store_info = Store::checkToken();
            if ($store_info instanceof Json){
            ...
            ##验证参数
            $res = $dynamic->scene('edit_dynamic_img')->check(input());
            if(!$res)throw new Exception($dynamic->getError());
            ##逻辑
            Db::startTrans();
            $data = $dynamicImg->editDynamicImg();
            Db::commit();
            ##返回
            return json(self::callback(1,'',$data));
        }catch(Exception $e){
     ...
     * @return array|false|\PDOStatement|string|\think\Model|Json
     */
    public function dynamicProductInfo(DynamicValidate $dynamic, DynamicModel $dynamicModel){
        try{
            ##token 验证
            $store_info = Store::checkToken();
            if ($store_info instanceof Json){
            ...
            ##验证参数
            $res = $dynamic->scene('dynamic_product_info')->check(input());
            if(!$res)throw new Exception($dynamic->getError());
            ##逻辑
            $data = $dynamicModel->getDynamicProInfo();
            ##返回
            return json(self::callback(1,'',$data));
        }catch(Exception $e){
            return json(self::callback(0,$e->getMessage()));
     ...
     * @return array|false|\PDOStatement|string|\think\Model|Json
     */
    public function editDynamicProductInfo(DynamicValidate $dynamic, DynamicModel $dynamicModel, DynamicProduct $dynamicProduct){
        ##接收参数
        $postJson = trim(file_get_contents('php://input'));
        $post = json_decode($postJson,true);
        ##token 验证
        ...
        try{
            Db::startTrans();
            ##验证参数
            $res = $dynamic->scene('edit_dynamic_product_info')->check($post);
            if(!$res)throw new Exception($dynamic->getError());
            ##逻辑
            ###修改打包购买状态
            $dynamicModel->editIsGroupBuy($post);

            $dynamic_id = (int)$post['id'];
            ###增加标签
            $dynamicProduct->del($dynamic_id);
            $res = $dynamicProduct->add($dynamic_id, $post['products'], [], $post['is_group_buy']);
            if(!$res)throw new Exception('操作失败');
