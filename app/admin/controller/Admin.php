...
class Admin extends Permissions
{
     ...
     * @return [type] [description]
     */
    public function admin_cate_publish()
    {
        //获取角色id
        $id = $this->request->has('id') ? $this->request->param('id', 0, 'intval') : 0;
        $model = new \app\admin\model\AdminCate();
        ...
        if($id > 0) {
            ...
            if($this->request->isPost()) {
                ...
                if(false == $model->allowField(true)->save($post,['id'=>$id])) {
                    ...
                    addlog($model->id);//写入日志
                    return $this->success('修改角色信息成功','admin/admin/admin_cate');
                }
            } else {
                //非提交操作
            ...
            if($this->request->isPost()) {
                ...
                if(false == $model->allowField(true)->save($post)) {
                    ...
                    addlog($model->id);//写入日志
                    return $this->success('添加角色成功','admin/admin/admin_cate');
                }
            } else {
                //非提交操作
                $menus = Db::name('admin_menu')->select();
     ...
     * @return [type] [description]
     */
    public function admin_cate_delete()
    {
        if($this->request->isAjax()) {
            $id = $this->request->has('id') ? $this->request->param('id', 0, 'intval') : 0;
            if($id > 0) {
                ...
                if(false == Db::name('admin_cate')->where('id',$id)->delete()) {
                    ...
                    addlog($id);//写入日志
                    return $this->success('删除成功','admin/admin/admin_cate');
                }
            } else {
                return $this->error('id不正确');
            }
