...
class Index extends Permissions
{
    ...
    {
        $menu = Db::name('admin_menu')->where(['is_display'=>1])->order(['orders'=>'asc','create_time'=>'asc'])->select();

        //删除不在当前角色权限里的菜单，实现隐藏
        $admin_cate = Session::get('admin_cate_id');

        //如果是超级管理员 id 1 则默认全部显示全部菜单
        if (Session::get('admin_cate_id') == 1) {
            $permissions = Db::name('admin_menu')->where('id','neq',708)->column('id');
        }else{
