...
class Logic
{
     ...
     * @return array|false|\PDOStatement|string|\think\Model
     */
    public static function getInvitation($invitation_code){
        return Db::name('user')->field('user_id,invitation_code')->where(['invitation_code'=>$invitation_code])->find();
    }
...
//        if($activity['status'] == 3)return returnArr(0,'活动已结束');
     ...
     * @return array|false|\PDOStatement|string|\think\Model
     */
    public static function getActivityBanner(){
        return Db::name('store_category_img')->where(['category_id'=>19])->field('id,img_url,type,content,link,product_id,store_id,chaoda_id')->select();
    }
    /**
    ...
    public static function getActivityBanners($banner_ids){
        return Db::name('activity_banner')->alias('ab')
            ->join('store s','ab.type=1 AND ab.link_id = s.id','LEFT')
            ->where(['ab.id'=>['IN', $banner_ids]])
            ->field(['ab.id','ab.img','ab.type','ab.link_id',
                'IF(ab.type=1 AND ab.link_id=s.id AND s.type=2,0,ab.type) type'
            ])
            ->order('ab.sort','asc')
            ->select();
    }
...
//        if($activity_info['status'] != 2)return returnArr(0,'活动未上线或者已下线');
        ...
        ##活动banner
        $banner_ids = explode(',', $activity_info['banner_ids']);
        $activity_banner = self::getActivityBanners($banner_ids);
        ...
        return returnArr(1,'',compact('list','activity_banner','activity','rule','activity_type'));
...
//        return Db::name('activity_product')->where(['activity_id'=>['IN',$ac_ids]])->column('product_id');
     ...
     * 获取买单商家设置员工提成
     * @param $store_id  ##店铺Id
     * @param $price_store ##店铺收益
     * @return int
     */
    public static function getStaffProfit($store_id, $price_store){
        $profit = Db::name('store')->where(['id'=>$store_id])->value('bussiness_deduct');
        return $profit ? $price_store*($profit/100) : 0;
    }
     ...
     * @return array
     */
    public static function getStoreReward($store_id, $staff_id, $price_yj){
        #获取员工累计经手买单金额
        $staff_maidan_total_money = (int)(Db::name('business')->where(['id'=>$staff_id])->value('maidan_total_money'));
        $cur_staff_maidan_total_money = $staff_maidan_total_money + $price_yj;
        $reward_list = Db::name('bussiness_reward')
            ->where([
                'store_id' => $store_id,
                'status' => 1,
                'conditions' => ['BETWEEN', [$staff_maidan_total_money + 0.01, $cur_staff_maidan_total_money]],
                'reward' => ['GT', 0]
            ])
            ->field('id,conditions,reward')
            ->select();
        if(!$reward_list)return [];
        ##销售总额阶梯奖励
        $reward = array_sum(array_column($reward_list,'reward'));
        return compact('reward','reward_list');
    }
     ...
     * @return int|mixed
     */
    public static function getNewUserReward($user_id){
        ##获取新用户定义时间
        $limit_time = config('config_common.new_user_limit_hour');
        $create_time = Db::name('user')->where(['user_id'=>$user_id])->value('create_time');
        ##判断新用户
        if($create_time - time() > ($limit_time * 24 * 24))return 0;
        ##判断是否有买单记录
        $check = Db::name('maidan_order')->where(['user_id'=>$user_id, 'status'=>2])->count('id');
        if($check)return 0;
        ##获取新用户奖励金额
        $reward = Db::name('maidan_config')->whereNull('delete_time')->order('create_time','desc')->value('new_user_reward');
        return $reward;
    }
     ...
     * @return int|mixed
     */
    public static function getFirstUserReward($staff_id){
        ##获取员工的推广用户数
        $invite_user_num = Db::name('maidan_order')->where(['staff_id'=>$staff_id, 'status'=>['GT', 1]])->count('id');
        if($invite_user_num)return 0;
        ##获取首个用户额外奖励
        $reward = Db::name('maidan_config')->whereNull('delete_time')->order('create_time','desc')->value('first_user_reward');
        return $reward;
    }
     ...
     */
    public static function getPlatformReward($staff_id){
        ##获取员工的推广用户数
        $invite_user_num = Db::name('business')->where(['id'=>$staff_id])->value('invite_user_num');
        $reward_list = Db::name('maidan_config')->alias('mc')
            ->join('maidan_platform_reward_rule mr','mc.id = mr.maidan_config_id','LEFT')
            ->whereNull('mc.delete_time')
            ->where('mr.person_num','>',$invite_user_num)
            ->where('mr.person_num','<=',($invite_user_num + 1))
            ->field('mr.id,mr.person_num,mr.reward')
            ->select();
        if(!$reward_list)return [];
        $reward = array_sum(array_column($reward_list,'reward'));
        return compact('reward','reward_list');
    ...
    /**
     * 新的待发货订单，告诉员工系统消息   您有新的待发货订单,请进入发货>     1
     */
    public function fahou_system_msg($store_id){
        //查看该店的登录未过期的员工
        //$business = Db::name('business')->where(['store_id'=>$store_id,'business_status'=>1,'token_expire_time'>time()])->field(['id','business_name'])->select();
        $business = Db::name('business')->where(['store_id'=>$store_id,'business_status'=>1,'token_expire_time'=>['egt',time()]])->field(['id','business_name'])->select();
        $business_msg_link_arr = [];
        $business_arr = [];
        foreach ($business as &$v){
            $arr = [];
            $arr['business_id'] = $v['id'];
            $arr['msg_id'] = 1;
            $arr['is_read'] = 0;
            $arr['create_time'] = time();
            array_push($business_msg_link_arr,$arr);
            $s_id = 's'.$v['id'];
            array_push($business_arr,$s_id);
        }
        //批量插入到系统消息
        $result   =  db('business_msg_link')->insertAll($business_msg_link_arr);
        if($result === false){
            return false;
        }
        //给在线的员工发送推送
        ...
        if(!empty($business_arr)){
            JiG::sendMsgToStaff($business_arr,'fahou_system_msg');
        }
        //}
        return true;
    }
    ...
    /**
     * 新的售后订单，告诉员工系统消息     您有新的售后订单，请进入处理>      2
     */
    public function shouhou_system_msg($store_id){
        //查看该店的登录未过期的员工
        //$business = Db::name('business')->where(['store_id'=>$store_id,'business_status'=>1,'token_expire_time'>time()])->field(['id','business_name'])->select();
        $business = Db::name('business')->where(['store_id'=>$store_id,'business_status'=>1,'token_expire_time'=>['egt',time()]])->field(['id','business_name'])->select();
        $business_msg_link_arr = [];
        ...
        //foreach ($business as &$v){
        if(!empty($business_arr)){
            JiG::sendMsgToStaff($business_arr,'shouhou_system_msg');
        }
        //}
        return true;
    }
    ...
    /**
     * 商品已售完，告诉员工系统消息    您的宝贝已售完，请前往下架>     3
     */
    public function sellout_system_msg($store_id,$product_id){
        //查看当前产品的库存
        $product_stocks = db('product_specs')->where(['product_id'=>$product_id])->field('sum(stock) as total_stocks')->find();
        //库存大于0,
        if($product_stocks['total_stocks'] > 0){
            return true;
        }
        //查看该店的登录未过期的员工
        $business = Db::name('business')->where(['store_id'=>$store_id,'business_status'=>1,'token_expire_time'=>['egt',time()]])->field(['id','business_name'])->select();
        ...
        //foreach ($business as &$v){
            if(!empty($business_arr)){
                JiG::sendMsgToStaff($business_arr,'sellout_system_msg');
            }
        //}
        return true;
    }
