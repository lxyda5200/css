...
class DynamicModel extends Model
{
    ...
    {
        $distance='IF('.$lat.' > 0 OR '.$lng.'>0, ROUND(lat_lng_distance('.$lat.','.$lng.',st.lat,st.lng),2), 0)';
        $list = Db::name('dynamic')->alias('d')
            ...
            ->field(['d.id','d.store_id','d.create_time','d.cover','d.title','d.scene_id','d.share_number','d.like_number','d.collect_number','d.comment_number','d.type',
                'st.store_name','st.cover as store_logo','st.address','st.signature','st.lng','st.lat','IF('.$lat.' > 0 OR '.$lng.'>0, ROUND(lat_lng_distance('.$lat.','.$lng.',st.lat,st.lng),2), 0) as distance',
    ...
    static public function GetStoreFollowDynamic($user_id,$page,$size,$lng,$lat)
    {
        ...
        $list = Db::name('dynamic')->alias('d')
            ...
            -> field([
                ...
                'st.store_name', 'st.cover as store_logo','st.address','st.signature','st.lng','st.lat',
                'IF('.$lat.' > 0 OR '.$lng.'>0, ROUND(lat_lng_distance('.$lat.','.$lng.',st.lat,st.lng),2), 0) as distance',
                't.title as topic_title',
...
//        $order = ['d.is_recommend'=>'DESC','weight'=>'DESC','d.create_time'=>'DESC'];
     ...
     *title
     */
    static public function GetPopularProducts($id,$derect)
    {
        if($id && $derect){
            $is_id=self::getPopularProductsId($id);
            if(!$is_id){return '没有找到传递的人气单品id或已下架!';}
            $sort=self::getPopularProductsSort($id,$derect);
        }else{
            $sort=self::getPopularProductsSort(0,3);
        }
        $where['sort']=$sort;
        $data=Db::name('popular_products')
...
//            ->find();
        if(!$data){return null;}
        $data['total_number']=Db::name('popular_products')
            ->field('id,title,bg_img,visit_num,create_time')
            ->where('status','EQ',1 )
            ->where('delete_time is null' )
            ->count();
        //查询绑定商品
        $data['product']=Db::name('popular_products_details')
            ->field('id,title,cover,desc,product_id,visit_num,pop_pro_id')
            ->where('pop_pro_id','EQ',$data['id'])
            ->select();
        if($data['product']){
    ...
    static public function liveCollectionList($user_id,$lng,$lat,$page,$size){
        $distance='IF('.$lat.' > 0 OR '.$lng.'>0, lat_lng_distance('.$lat.','.$lng.',s.lat,s.lng), 0)';
        $list = Db::name('dynamic_collection')
            ...
            -> field(['d.id as d_id','d.title','d.type','d.description','d.cover as dynamic_cover','d.status','d.look_number','d.scene_id','d.share_number','d.like_number',
                ...
                'IF(dd.create_time > 0 ,1 ,0) is_dianzan',
                't.title as topic','sc.title as scene','ROUND('.$distance.',2) as distance'])
