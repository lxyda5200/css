 ...
 * @return array
 */
function Recommend($id,$look,$like,$share,$distance){

    $quanzhong=($look*0.1+$like*0.3+$share*0.35+$distance*0.25)/4;
 ...
 * 距离
 */
function GetDynamicDistance($lat,$lng){
		...
		6378.138 * 2 * ASIN(
			SQRT(
				POW(
					SIN(
						(
							{$lat} * PI() / 180 - st.lat * PI() / 180
						) / 2
					),
					2
				) + COS({$lat} * PI() / 180) * COS(st.lat * PI() / 180) * POW(
					SIN(
						(
							{$lng} * PI() / 180 - st.lng * PI() / 180
						) / 2
					),
					2
				)
