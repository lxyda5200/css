...
use app\user_v6\controller\Interact;
use app\user_v6\common\Logic;
use app\user_v6\common\User as UserFunc;
use app\user_v6\common\UserLogic;
use app\user_v6\model\DrawLottery;
use app\user_v6\model\DynamicModel;
use app\user_v6\model\Giftpack;
use app\user_v6\model\GiftpackOrder;
use app\user_v6\model\MemberOrder;
use app\user_v6\model\Product as Pmodel;
use app\user_v6\model\ProductComment;
use app\user_v6\validate\ProductValidate;
use think\Cache;
use think\Config;
use think\Db;
...
use app\user_v6\validate\CouponValidate;
...
//            }
            ...
            //是否是查询
                if($category){
                    ...
                    if($ids1 && !$ids2 && !$ids3){
                        $ids=$ids1;
                    }elseif ($ids1 && $ids2 && !$ids3){
                        $ids=array_unique(array_merge($ids1,$ids2));
                    }elseif ($ids1 && !$ids2 && $ids3){
                        $ids=array_unique(array_merge($ids1,$ids3));
                    }elseif ($ids1 && $ids2 && $ids3){
                        $ids=array_unique(array_merge($ids1,$ids2,$ids3));
                    }elseif (!$ids1 && $ids2 && !$ids3){
                        $ids=$ids2;
                    }elseif (!$ids1 && $ids2 && $ids3){
                        $ids=array_unique(array_merge($ids2,$ids3));
                    }elseif (!$ids1 && !$ids2 && $ids3){
                        $ids=$ids3;
                    }
                }
...
//            }
           // $total = Db::name('store')->where('category_id',$category_id)->where($where)->count('id');
            $total = Db::name('store')
                ->field(['id','read_number','share_number','cover','store_name','description','dianzan','brand_name','address','lat_lng_distance('.$lat.','.$lng.',store.lat,store.lng) as distance','collect_number'.$field])
//                ->where('category_id',$category_id)
                ->where($where)
                ->count();
            $list = Db::name('store')
                ->field(['id','read_number','share_number','cover','store_name','lng','lat','description','dianzan','brand_name','address','lat_lng_distance('.$lat.','.$lng.',store.lat,store.lng) as distance','collect_number'.$field])
//                ->where('category_id',$category_id)
                ->where($where)
                ->page($page,$size)
                ->order("score","desc")
    ...
    public function nearbyStore(){
        try {
            ...
            if($brand_name){
                ...
                $list = Db::name('store')
                    ->field(['id','read_number','share_number','cover','store_name','lng','lat','description','dianzan','brand_name','address','lat_lng_distance('.$lat.','.$lng.',lat,lng) as distance','collect_number'])
...
//                $range = 10000000;	//距离  km
                    $range = 3;	//距离  km
                    $wh = " lat_lng_distance($lat,$lng,lat,lng) <= $range  ";
                }
                $total = Db::name('store')
                    ->where('type',1)
                    ->where('sh_status',1)
...
//            $data['link'] = request()->domain() . "/index.php/user/index/product_content_p/id/{$data['id']}.html";
            $data['link'] = "http://121.196.214.146/css/public/index.php/user_v6/index/product_content_p/id/{$data['id']}.html";
            $data['product_img'] = Db::name('product_img')->where('product_id',$product_id)->column('img_url');
        ...
        try{
            ...
            if($card_info){
                    ...
                    if($order_id){
                        switch($pay_type){
                            case 1:
                                $notify_url = SERVICE_FX."/user_v6/ali_pay/member_alipay_notify";
                                $aliPay = new AliPay();
                                $data['alipay_order_info'] = $aliPay->appPay($order_no,$pay_money,$notify_url);
                                break;
                            case 2:
                                $notify_url = SERVICE_FX."/user_v6/wx_pay/member_wxpay_notify";
                                $wxPay = new WxPay();
...
//            $search_type    = input('search_type',1,'intval');
            ...
            if($type == 1){
                ...
                $list = Db::name('store')->field(['id','read_number','cover','store_name','lng','lat','description','dianzan','collect_number','brand_name','address','lat_lng_distance('.$lat.','.$lng.',store.lat,store.lng) as distance','type as store_type'])
                    ->where($where)
                    ->where('sh_status',1)
                    ->where('store_status',1)
                    ->where('type',1)
