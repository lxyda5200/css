...
use app\user_v6\model\DynamicCommentDianzanModel;
use app\user_v6\model\DynamicModel;
use app\user_v6\model\DynamicDianzanModel;
use app\user_v6\model\DynamicCollectionModel;
use app\user_v6\model\NewTrendModel;
use app\user_v6\model\NewTrendDianzanModel;
use app\user_v6\model\NewTrendCollectionModel;
use app\user_v6\model\NewTrendCommentModel;
use think\Cache;
use think\Db;
...
class Dynamic extends Base
{
    ...
    public function PopularProducts(){
        try{
            ...
            $data = DynamicModel::GetPopularProducts($id,$derect);
            if(is_string($data)) return json(self::callback(0, $data), 400);
            if ($data === false) return json(self::callback(0, '失败!'), 400);
