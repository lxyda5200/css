...
use app\user_v6\common\Orders;
use app\user_v6\common\StaffLogic;
use app\user_v6\common\UserLogic;
use app\user_v6\model\ProductOrder;
use app\user_v6\model\ProductShouhou as productshouhouModel;
use jiguang\JiG;
use think\Db;
...
use app\user_v6\validate\User as UserValidate;
...
//                    'coupon_money' => round(($coupon_money/$store_number)),
            ...
            switch($pay_type){
                case 1:
                    $pay_type = "支付宝";
                    $notify_url = SERVICE_FX."/user_v6/ali_pay/goods_alipay_notify";
                    $aliPay = new AliPay();
                    $data['alipay_order_info'] = $aliPay->appPay($pay_order_no,$pay_money,$notify_url);
                    break;
                case 2:
                    $pay_type = "微信";
                    $notify_url = SERVICE_FX."/user_v6/wx_pay/goods_wxpay_notify";
                    $wxPay = new WxPay();
    ...
    public function Pay(){
        try{
            ...
            switch($pay_type){
                case 1:
                    $pay_type = "支付宝";
                    $notify_url = SERVICE_FX."/user_v6/ali_pay/repay_alipay_notify";
                    $aliPay = new AliPay();
                ...
                case 2:
                    $pay_type = "微信";
                    $notify_url = SERVICE_FX."/user_v6/wx_pay/repay_wxpay_notify";
                    $wxPay = new WxPay();
    ...
    public function cancel(){
        try{
            ...
            if ($order_status == 1){  //待支付->取消订单
                ...
                ##消息推送
                JiG::sendMsgToUser("u{$userInfo['user_id']}", 'order_refund', ['order_id'=>$order_id, 'order_no'=>$order->order_no]);
            }elseif($order_status == 2){
                return \json(self::callback(0,'正在拼单中不能取消'));
            }else{
                return \json(self::callback(0,'该订单不支持此操作'));
...
//                    ->count();
                ...
                $list = Db::name('product_shouhou ')->alias('ps')
                    ->join('product_order_detail p','ps.order_id = p.order_id AND ps.specs_id = p.specs_id','LEFT')
                    ->join('product_order po','p.order_id = po.id','LEFT')
                    ->join('product pr','p.product_id = pr.id','LEFT')
                ...
                p.cover,pr.product_name,p.product_id,p.specs_id,p.product_specs,p.number,p.price,p.is_shouhou,s.store_name,p.status,s.cover as store_logo,p.is_refund,p.refund_time,p.refund_money,p.realpay_money,ps.refuse_description,ps.return_mode,ps.create_time,ps.refund_type,ps.goods_status,ps.refund_status,ps.refund_reason')
                    ->where('ps.id','in',$ids)
                ...
                foreach ($list as $k=>$v){
                    ...
                    if($v['order_status']==8){
                        $list[$k]['order_status']=6;//已结转归为已完成
                    }
                }
                //返回售后类型1
                $data['is_shouhou']=1;
                ...
                foreach ($list as $k=>$v) {
                    ...
                    $product_info = Db::view('product_order_detail','product_id,specs_id,product_specs,number,realpay_money,status,price,platform_price,is_comment,is_shouhou,is_refund')
                        ->view('product','product_name','product_order_detail.product_id=product.id','LEFT')
                    ...
                    foreach ($product_info as $k2=>$v2){
                       ...
                       // $product_info[$k2]['price'] = $v2['realpay_money']/$v2['number']+ $v2['platform_price'] ;
                        ...
                        if($v['order_status']==8){
                            $product_info[$k2]['order_status']=6;
                        }
                    }
...
//                    ->where($where4)
                ...
                foreach ($list as $k=>$v) {
                    ...
                    $product_info = Db::view('product_order_detail','product_id,specs_id,product_specs,status,number,price,platform_price,realpay_money,is_comment,is_shouhou,is_refund')
                        ->view('product','product_name','product_order_detail.product_id=product.id','LEFT')
                        ->view('product_specs','cover','product_specs.id = product_order_detail.specs_id','LEFT')
                        ->where('product_order_detail.order_id',$v['order_id'])
                        ->select();
    ...
    $product_info = Db::view('product_order_detail','order_id,product_id,specs_id,product_specs,number,price,is_comment,is_shouhou,status,is_refund,realpay_money,comment_standard_time,finish_standard_time')
        ->view('product','product_name','product_order_detail.product_id = product.id','LEFT')
...
//            $goods_status = input('goods_status');
           ...
           // $refund_times = input('refund_times') ? intval(input('refund_times')) : 1 ;//次数
            ...
            //给商家发送推送以及系统消息   售后申请
            Logic::shouhou_system_msg($order['store_id']);
    ...
    public function logistics_info(){
        try {
            ...
            $rst = Db::name('product_shouhou')->where('id',$shouhou['id'])->update($genxin);
            if($rst===false){
    ...
    public function refundDetail(){
        try {
            ...
            $refund_info = Db::view('product_order_detail','order_id,product_id,specs_id,product_specs,cover,status,number,price,is_comment,is_shouhou,is_refund,realpay_money')
                ->view('product','product_name','product_order_detail.product_id=product.id','LEFT')
...
//                ->where('coupon_rule.end_time','gt',$time2)
    ...
    public function maidanOrder(UserValidate $UserValidate){
        ...
        try{
            ...
            switch($pay_type){
                case 'wx':
                    $notify_url = SERVICE_FX."/user_v6/wx_pay/maidan_wxpay_notify";
                    $wxPay = new WxPay();
                    $data['wxpay_order_info'] = $wxPay->getOrderSign($order_sn,$price_maidan,$notify_url);
                    break;
                case 'zfb':
                    $notify_url = SERVICE_FX."/user_v6/ali_pay/maidan_alipay_notify";
                    $aliPay = new AliPay();
...
//            if($price_maidan < 0.01) return \json(self::callback(0,'支付金额不能小于0.01元'));
        ...
        ##计算平台优惠金额
        $platform_policy = abs(round($price_yj_2 - $price_maidan - $store_policy,2));
        ...
        try{
            ...
            $staff_id = input('post.staff_id',0,'intval');
            $staff_profit = $achievement_store_reward = $new_user_reward = $first_user_reward = $achievement_platform_reward = 0;
            if($staff_id){
                ##计算员工提成
                $staff_profit = Logic::getStaffProfit($store_id, $price_store);
                if($staff_profit){
                    $staff_profit_data = [
                        'staff_id' => $staff_id,
                        'store_id' => $store_id,
                        'price_profit' => $staff_profit,
                        'type' => 1,
                        'create_time' => time()
                    ];
                }
                ##计算员工商家阶梯奖励
                $store_reward = Logic::getStoreReward($store_id, $staff_id, $price_yj);
                if($store_reward){
                    $achievement_store_reward = $store_reward['reward'];
                    ##员工商家阶梯奖励收益记录
                    $store_reward_data = [];
                    foreach($store_reward['reward_list'] as $v){
                        $store_reward_data[] = [
                            'type' => 2,
                            'price_profit' => $v['reward'],
                            'achievement_price' => $v['conditions'],
                            'store_id' => $store_id,
                            'staff_id' => $staff_id,
                            'create_time' => time()
                        ];
                ...
                ##计算单新用户推广奖励、首个用户额外奖励
                $new_user_reward = Logic::getNewUserReward($user_id);
                if($new_user_reward){
                    $new_user_reward_data = [
                        'staff_id' => $staff_id,
                        'store_id' => $store_id,
                        'price_profit' => $new_user_reward,
                        'type' => 3,
                        'create_time' => time()
                    ];
                }else{
                ...
                $first_user_reward = Logic::getFirstUserReward($staff_id);
                if($first_user_reward){
                    $first_user_reward_data = [
                        'staff_id' => $staff_id,
                        'store_id' => $store_id,
                        'price_profit' => $first_user_reward,
                        'type' => 4,
                        'create_time' => time()
                    ];
                }else{
                ...
                ##计算平台阶梯奖励
                $price_limit = (float)(Db::name('maidan_config')->whereNull('delete_time')->order('create_time','desc')->value('min_maidan_price'));
                if($price_limit < $price_yj){
                    $achievement_platform_reward2 = Logic::getPlatformReward($staff_id);
                    if($achievement_platform_reward2){
                        $achievement_platform_reward = $achievement_platform_reward2['reward'];
                        $platform_reward_data = [];
                        foreach($achievement_platform_reward2['reward_list'] as $v){
                            $platform_reward_data[] = [
                                'staff_id' => $staff_id,
                                'store_id' => $store_id,
                                'achievement_user_num' => $v['person_num'],
                                'price_profit' => $v['reward'],
                                'type' => 5,
                                'min_maidan_price' => $price_limit,
                                'create_time' => time()
                            ];
                        }
                    }
            ...
            ##最终商家收益 = 商家收益 - 员工提成 - 员工商家阶梯奖励
            $price_store = $price_store - $staff_profit - $achievement_store_reward;
            if($price_store <= 0)throw new Exception('支付金额过低');

            $data = compact('price_yj','price_maidan','user_id','store_id','discount','is_member','order_sn','pay_type','member_order_id','coupon_id','coupon_money','platform_profit','price_store','discount_platform','platform_policy','store_policy','user_mobile','staff_profit','achievement_store_reward','new_user_reward','first_user_reward','achievement_platform_reward','staff_id');
            $data['create_time'] = time();
            $res = UserLogic::addMaidanOrder($data);
            if($res === false)throw new Exception('订单生成失败');
            $data['order_id'] = $res;
            ...
            $maidan_order_id = $res;
            ##增加员工受益记录
            if(isset($staff_profit_data) && $staff_profit_data){
                $staff_profit_data['maidan_order_id'] = $maidan_order_id;
                StaffLogic::addProfitRecord($staff_profit_data);
            }
            if(isset($store_reward_data) && $store_reward_data){
                StaffLogic::addProfitRecords($store_reward_data, $maidan_order_id);
            }
            if(isset($new_user_reward_data) && $new_user_reward_data){
                $new_user_reward_data['maidan_order_id'] = $maidan_order_id;
                StaffLogic::addProfitRecord($new_user_reward_data);
            }
            if(isset($first_user_reward_data) && $first_user_reward_data){
                $new_user_reward_data['maidan_order_id'] = $maidan_order_id;
                StaffLogic::addProfitRecord($first_user_reward_data);
            }
            if(isset($platform_reward_data) && $platform_reward_data){
                StaffLogic::addProfitRecords($platform_reward_data, $maidan_order_id);
            }
...
//            throw new Exception('恭喜');
    ...
    public function submitOrder(){
        try {
            ...
            $store_info = $post['store_info'];   //商品信息
            $pay_type = $post['pay_type'] ? intval($post['pay_type']) : '';      //收货地址
            $dynamic_id = isset($post['dynamic_id'])?intval($post['dynamic_id']):0;  //动态id
            ...
            foreach ($store_info as $k=>$v){
                ...
                foreach ($product_info as $k2=>$v2) {
                    ...
                    ##动态打包购买
                    if($dynamic_id>0){
                        $price = Db::name('dynamic_product_specs')->where(['dynamic_id'=>$dynamic_id, 'specs_id'=>$v2['specs_id']])->value('price');
                    }
...
//                    'coupon_money' => round(($coupon_money/$store_number)),
            ...
            if(!$chaoda_id && !$dynamic_id){  //潮搭不参加活动
                ##判断是否参与了有效活动
                $is_activity = false;
...
//        ];
        ...
        try{
            ...
            #逻辑
            $rtn_data = [];
            if(!$params['is_chaoda'] && !$params['is_dynamic']){ ##非潮搭 &&  非动态打包购买
                ##获取商品id 和 商品券/店铺券 ID
