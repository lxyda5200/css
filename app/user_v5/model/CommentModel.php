...
class CommentModel extends Model
{
    ...
    public function reply(){
        return $this->hasMany('CommentModel', 'pid', 'id')
            ...
            -> field('dynamic_comment.content,dynamic_comment.dynamic_id,dynamic_comment.pid,dynamic_comment.id,dynamic_comment.user_id,user.nickname,user.avatar,dynamic_comment.b_user_id,b.nickname as b_user_nickname,b.avatar as b_user_avatar');
    }
    ...
    public static function detailsCommentPage($user_id,$dynamic_id,$page,$size){
        ...
        // 分页查询评论数据
        $data = self::where(['d.dynamic_id' => $dynamic_id, 'd.pid' => 0])
            -> alias('d')
            -> field([
                'd.id', 'd.pid', 'd.create_time', 'd.user_id', 'd.content', 'd.dynamic_id',
                'u.nickname', 'u.avatar',
    ...
    public static function userCommentRecoveryList($user_id){
        ...
        $list = (new self())->where(['b_user_id'=>$user_id])->field('id,user_id,content,pid,create_time')->order('create_time','desc')->limit(($page-1)*$size,$size)->select();
