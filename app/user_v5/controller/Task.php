...
//            if($res === false)throw new Exception("\n\r" . "待发货自动取消订单返还优惠券[店铺]失败=>订单IDS【". json_encode($order_ids) ."}】,优惠券IDS=>【"  . json_encode($store_coupon_ids) ."】");
            ...
            foreach($list as $v){
                ...
                ##获取订单需要退款的金额
                $refund_money = UserLogic::getOrderCanRefundMoney($v['id'] ,$v['total_freight']);

                if($v['pay_type'] == '支付宝'){
                    $alipay = new AliPay();
                    $res = $alipay->alipay_refund($order_no,$v['id'],$refund_money);
                }else if($v['pay_type'] == '微信'){
                    $total_pay_money = $v['pay_scene'] ? $v['pay_money'] :UserLogic::orderTotalPayMoney($v['pay_order_no']);
                    $wxpay = new WxPay();
                    $res = $wxpay->wxpay_refund($order_no,$total_pay_money,$refund_money);
                }
    ...
    public function autoFinishOrder(){
        ...
        try{
            $store_shouru = $dg_money = $user_money = $rtn_coupon = $rtn_money = $order_ids = $detail_ids = $huoli_detail_ids = $product_sale = $specs_sale = $freight = [];
            ...
            foreach($list as $v){
                ...
                ##减去每个商品的平台提成
                $store_shouru[$v['store_id']] -= $v['platform_profit'];

                ##运费(第一个商品结转时结转运费)
                if(!UserLogic::checkFinishFreight($v['order_id'])){
                    $freight[$v['store_id']][$v['order_id']] = $v['total_freight'];
                }
    ...
    public function autoReceiveShouHouPro(){
        ...
        foreach($list as $v){
            ...
            try{
                ...
                ##修改订单商品详情的状态
                $res = UserLogic::editProOrderDetailShouhou($v['detail_id'], $v['realpay_money']);
                ...
                if($v['realpay_money'] >= 0.01){
                    //todo 此处原路退款
                    if ($v['pay_type'] == '支付宝') {
                        $alipay = new AliPay();
                        $res = $alipay->alipay_refund($pay_order_no,$v['detail_id'],$v['realpay_money']);
                    }elseif ($v['pay_type'] == '微信'){
                        $total_pay_money = $v['pay_scene']?$v['pay_money']: UserLogic::sumOrderPayMoney($pay_order_no);
                        $wxpay = new WxPay();
                        $res = $wxpay->wxpay_refund($pay_order_no,$total_pay_money,$v['realpay_money']);
                    }
                    if ($res !== true){
