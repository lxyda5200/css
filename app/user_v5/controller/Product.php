...
use app\user_v5\model\Giftpack;
use app\user_v5\model\GiftpackOrder;
use app\user_v5\model\MemberOrder;
use app\user_v5\model\Product as Pmodel;
use app\user_v5\model\ProductComment;
use app\user_v5\validate\ProductValidate;
use think\Cache;
use think\Config;
use think\Db;
...
use app\user_v5\validate\CouponValidate;
class Product extends Base
{
    ...
    public function recommendList(){
        try{
            $data = Db::name('recommend_banner')->field('id,title,description,image,activity_id,type')->where('status',1)->order('sort','asc')->select();
...
//                $range = 10000000;	//距离  km
        ...
        6378.138 * 2 * ASIN(
            ...
            if($brand_name){
                ...
                $list = Db::name('store')
                    ->field('id,read_number,cover,store_name,brand_name,dianzan,collect_number,description,address,lng,lat')
...
//                $list[$k]['collect_number'] = Db::name('store_collection')->where(['store_id'=>$v['id']])->count('id');
    ...
    public function storeDetail(){
        ...
        try {
            ...
            $data = Db::name('store')->field('id,cover,store_name,type,buy_type,telephone,description,is_ziqu,province,city,area,address,lng,lat,read_number,maidan_deduct')->where('id',$store_id)->find();
            if($member){
            ...
            $coupons = Db::name('coupon_rule')
                ...
                ->field('id,coupon_name,satisfy_money,coupon_money,start_time,end_time,coupon_type,kind,type,days')
                ->select();
...
//            $data = Cache::get('product_'.$product_id.$product_specs);
            ...
            $data = Db::view('product','id,category_id,product_name,sales,store_id,freight,is_buy,content,is_group_buy,start_time,end_time,huoli_money,type,see_type,fake_collect_num as product_collection_number,buy_type,share_price,video,is_zdy_price,product_type,days')
                ...
                ->find();
            //查询商品收藏数
//            $data['collection_number']=Db::name('product_collection')->where('product_id',$product_id)->count();
...
//            $data['link'] = request()->domain() . "/index.php/user/index/product_content_p/id/{$data['id']}.html";
            $data['link'] = "http://121.196.214.146/css/public/index.php/user_v5/index/product_content_p/id/{$data['id']}.html";
            $data['product_img'] = Db::name('product_img')->where('product_id',$product_id)->column('img_url');
        ...
        try{
            ...
            if($card_info){
                    ...
                    if($order_id){
                        switch($pay_type){
                            case 1:
                                $notify_url = SERVICE_FX."/user_v5/ali_pay/member_alipay_notify";
                                $aliPay = new AliPay();
                                $data['alipay_order_info'] = $aliPay->appPay($order_no,$pay_money,$notify_url);
                                break;
                            case 2:
                                $notify_url = SERVICE_FX."/user_v5/wx_pay/member_wxpay_notify";
                                $wxPay = new WxPay();
...
//            $search_type    = input('search_type',1,'intval');
            ...
            if($type == 1){
                ...
                $list = Db::name('store')->field('id,read_number,cover,store_name,description,dianzan,collect_number,brand_name,address,type as store_type')
                    ->where($where)
                    ->where('sh_status',1)
                    ->where('store_status',1)
                    ->where('type',1)
