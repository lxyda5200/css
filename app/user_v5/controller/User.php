...
use app\user_v5\common\Login;
use app\user_v5\model\Coupon;
use app\user_v5\model\HouseEntrust;
use app\user_v5\validate\Login as LoginValidate;
use app\user_v5\common\Login as LoginFunc;
use app\user_v5\common\User as UserFunc;
use app\user_v5\validate\UserAddress;
use jiguang\JiG;
use think\Db;
use think\Exception;
use think\Log;
...
use app\user_v5\validate\UserAddress as UserAddrValidate;
use app\user_v5\model\UserAddress as UserAddrModel;
use app\user_v5\model\House as HouseModel;
use app\user_v5\model\HouseShort as HouseShortModel;
use app\user_v5\model\Store as StoreModel;
use app\user_v5\common\UserLogic;
use app\user_v5\controller\Task as TaskCon;
 ...
 * */
    public function store_collection_and_cancel(){
        try{
            //token 验证
            $userInfo = \app\user_v5\common\User::checkToken();
            if ($userInfo instanceof Json){
                return $userInfo;
            }
            $param = $this->request->post();
    ...
    public function maidanCouponList(UserValidate $UserValidate){
        try{
            ...
            $list = Db::name('coupon')->alias('c')
                ...
                ->where($where)
                ->where(['cr.client_type'=>['IN',[0,2]],'cr.use_type'=>['IN',[0,1,3]]])  //卡券的适用平台限制
                ->field('c.id,cr.coupon_name,cr.satisfy_money,cr.can_stacked,cr.coupon_money,c.expiration_time,cr.coupon_type,cr.kind,cr.type,s.store_name,p.product_name,cr.is_superposition,cr.store_id,cr.coupon_name')
    ...
    public function test(){
        ...
        JiG::sendMsgToUser(["u13034"],"order_wait_pay_60");
//        JiG::sendMsgToUser(["u15239","u13042"],"测试推送",'马勇西瓜哥你们好');
//        JiG::getUserInfo(15234);
//        JiG::registerService(90);
//        JiG::editUserInfo('15234');
//       echo Db::name('user')->where(['id'=>['GT',1]])->where(['id'=>['LT',10]])->fetchSql(true)->select();
