...
use think\Exception;
use think\Log;
use app\store_v1\controller\Task as TaskCon;
class Logic
{
    ...
    public static function storeInfo($store_id){
        return Db::name('store_v1')->where(['id'=>$store_id])->field('id,store_status,mobile')->find();
    }
    ...
    public static function storeMoney($store_id){
        return Db::name('store_v1')->where('id',$store_id)->value('money');
    }
    ...
    public static function IncStoreMoney($store_id,$money){
        return Db::name('store_v1')->where(['id'=>$store_id])->setInc('money',$money);
    }
...
//            ->field('')
    ...
    public static function executeTask(){
        try{
            $auto_time_message = Cache::get('auto_carry_money',0);
            if(!$auto_time_message || (time() - $auto_time_message > 30 * 60)){
                $task = new TaskCon();
                ##自动取消订单
                $task->AutoCarryMoney();
                Cache::set('auto_carry_money',time());
            }
        }catch(Exception $e){
     ...
     * @return
     */
    public static function GetCarryList(){
        $endYesterday=mktime(0,0,0,date('m'),date('d'),date('Y'))-1;
        return Db::name('coupon_validate')
            ->where('status',1)
            ->where('finish_time',0)
            ->where('create_time','<',$endYesterday)
            ->field('id,store_id,platform_price')
            ->select();
    }
    /**
     * 更新核销记录为已结转
     * @param $coupon_validates
     * @return int|string
     * @throws Exception
     * @throws \think\exception\PDOException
     */
    public static function Carryed($coupon_validates){
        return Db::name('coupon_validate')->where(['id'=>['IN',$coupon_validates]])->update(['status'=>2,'finish_time'=>time()]);
    }
    /**
     * 增加商家余额
     * @return
     */
    public static function AutoMoney($id,$store_id,$platform_price){
        //查询商家余额
        $balance=Db::name('store_v1')
            ->where('id',$store_id)
            ->value('money');
        $newmoney=$balance+$platform_price;
            if($platform_price>0){
                $data = [
                    'store_id' => $store_id,
                    'money' => $platform_price,
                    'balance' => $newmoney,
                    'note' => '平台补贴线下优惠券金额自动结转给商家',
                    'coupon_validate_id' => $id,
                    'create_time' => time()
                ];
                $rst= Db::name('store_money_detail')->insertGetId($data);
            }else{
                return true;
            }
       if($rst){
           return Db::name('store_v1')->where('id',$store_id)->setField('money',$newmoney);
       }else{
           return false;
       }
    }
