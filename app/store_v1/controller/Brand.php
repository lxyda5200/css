...
use app\store_v1\model\BrandDynamic;
use app\store_v1\model\BrandDynamicAds;
use app\store_v1\model\BrandProduct;
use app\store_v1\model\BrandStore;
use app\store_v1\model\BrandStory;
use app\store_v1\model\BrandStoryAds;
use app\store_v1\validate\BrandDynamicArticle;
use think\Db;
use think\Exception;
use think\Request;
...
//            }
            ...
            if(empty($params['store_id'])){
                $params['store_id'] = $this->store_info['store_id'];
            }
            # 获取品牌id
    ...
    public function saveBrandDynamicAds(BrandDynamic $brandDynamic, BrandDynamicAds $ads) {
        ...
        try{
            ...
            # 数据验证
//            $validate = new \app\store_v1\validate\BrandDynamicAds();
...
//                throw new Exception($validate->getError());
            ...
            if(!$res) {
                ...
                Db::commit();
                return \json(self::callback(1, '操作成功', ['id' => $res]));
            }
            # 更新
            $dynamic_id = Db::table('brand_store')->alias('bs')
                ->join(['brand_dynamic' => 'bd'], 'bs.brand_id=bd.brand_id')
                ->where(['bs.store_id' => $params['store_id'], 'bs.type' => 2])->value('bd.id');
            $params['brand_dynamic_id'] = $dynamic_id;
            unset($params['token']);
            unset($params['store_id']);
    ...
    public function changeDynamicStatus(BrandStore $store) {
        try{
            ...
            return \json(0, $exception->getMessage());
        }
    }
}