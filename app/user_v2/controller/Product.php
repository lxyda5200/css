...
use app\user_v2\model\Giftpack;
use app\user_v2\model\GiftpackOrder;
use app\user_v2\model\MemberOrder;
use app\user_v2\model\Product as Pmodel;
use app\user_v2\model\ProductComment;
use app\user_v2\validate\ProductValidate;
use think\Cache;
use think\Db;
    ...
    public function storeDetail(){
        ...
        try {
            ...
            $coupons = Db::name('coupon_rule')
                ->where(['store_id'=>$store_id,'type'=>2,'is_open'=>1])
                ->where(['start_time'=>['ELT',time()],'end_time'=>['GT',time()],'surplus_number'=>['GT',0]])
                ->where(['client_type'=>['IN',[0,2]],'use_type'=>['IN',[1,3]]])  //卡券的适用平台限制
...
//                ->where('category_id',$category_id)
            ...
            $list = Db::view('product','id,category_id,product_type,days,huoli_money,is_buy')
                ->view('product_specs','id as specs_id,product_name,product_specs,price,huaxian_price','product_specs.product_id = product.id','left')
...
//            $data['link'] = request()->domain() . "/index.php/user/index/product_content_p/id/{$data['id']}.html";
            $data['link'] = "http://121.196.214.146/css/public/index.php/user_v2/index/product_content_p/id/{$data['id']}.html";
            $data['product_img'] = Db::name('product_img')->where('product_id',$product_id)->column('img_url');
        ...
        try{
            ...
            if($card_info){
                    ...
                    if($order_id){
                        switch($pay_type){
                            case 1:
                                $notify_url = SERVICE_FX."/user_v2/ali_pay/member_alipay_notify";
                                $aliPay = new AliPay();
                                $data['alipay_order_info'] = $aliPay->appPay($order_no,$pay_money,$notify_url);
                                break;
                            case 2:
                                $notify_url = SERVICE_FX."/user_v2/wx_pay/member_wxpay_notify";
                                $wxPay = new WxPay();
