...
use app\user_v2\common\Orders;
use app\user_v2\common\UserLogic;
use app\user_v2\model\ProductOrder;
use think\Db;
use app\user_v2\common\User;
use think\Exception;
use think\Log;
...
//                    'coupon_money' => round(($coupon_money/$store_number)),
            ...
            switch($pay_type){
                case 1:
                    $pay_type = "支付宝";
                    $notify_url = SERVICE_FX."/user_v2/ali_pay/goods_alipay_notify";
                    $aliPay = new AliPay();
                    $data['alipay_order_info'] = $aliPay->appPay($pay_order_no,$pay_money,$notify_url);
                    break;
                case 2:
                    $pay_type = "微信";
                    $notify_url = SERVICE_FX."/user_v2/wx_pay/goods_wxpay_notify";
                    $wxPay = new WxPay();
    ...
    public function Pay(){
        try{
            ...
            switch($pay_type){
                case 1:
                    $pay_type = "支付宝";
                    $notify_url = SERVICE_FX."/user_v2/ali_pay/repay_alipay_notify";
                    $aliPay = new AliPay();
                ...
                case 2:
                    $pay_type = "微信";
                    $notify_url = SERVICE_FX."/user_v2/wx_pay/repay_wxpay_notify";
                    $wxPay = new WxPay();
        ...
        try{
            ...
            if ($order_status == 1){  //待支付->取消订单
                ...
                if($store_coupon_id){
                    ...
                    if($res === false)throw new Exception('取消订单返还优惠券失败');
                }
                $coupon_id = $order->coupon_id;
                if($coupon_id){
    ...
    public function orderList(){
        try{
            ...
            if($order_status==7){
                ...
                $list = Db::view('product_order','id,order_no,pay_order_no,store_id,pay_money,total_freight,order_status,address_status,pt_id,order_type,pt_type')
                    ->view('store','store_name,cover,type,type as store_type','store.id = product_order.store_id','left')
                    ->where('product_order.user_id',$userInfo['user_id'])
...
//                ->where('coupon_rule.end_time','gt',$time2)
    ...
    public function maidanOrder(UserValidate $UserValidate){
        ...
        try{
            ...
            switch($pay_type){
                case 'wx':
                    $notify_url = SERVICE_FX."/user_v2/wx_pay/maidan_wxpay_notify";
                    $wxPay = new WxPay();
                    $data['wxpay_order_info'] = $wxPay->getOrderSign($order_sn,$price_maidan,$notify_url);
                    break;
                case 'zfb':
                    $notify_url = SERVICE_FX."/user_v2/ali_pay/maidan_alipay_notify";
                    $aliPay = new AliPay();
...
//                print_r($store);die;
                ...
                if(!$coupon_info['coupon_money'] && !$coupon_info['satisfy_money'] && $store['store_coupon_id'])throw new Exception('当前店铺优惠券已不可使用');
                if($price_tt < $coupon_info['satisfy_money'])throw new Exception('店铺优惠券不满足使用金额');
                ##如果优惠券金额大于本单支付总金额
                if($price_tt < $coupon_info['coupon_money']){
                    ...
                    ##店铺第一次均摊实付金额
                    $v['data']['pay_money'] = $price_tt - $coupon_info['coupon_money'] + $v['data']['total_freight'];  //加上运费的金额
                ...
                foreach($v['product'] as $kk => &$vv){
                    ...
                    if($price_tt < $coupon_info['coupon_money']){  //优惠券金额大于订单总价
                        ...
                        if(count($v['product']) <= $num){  ##最后一个产品
                            ...
                            $vv['realpay_money'] = $price_t - $rest_coupon_money;
                        }else{  ##前几个产品
           ...
           // Log::info(print_r($coupon_data,true));
            ...
            if($coupon_id){
                ...
                if($coupon_info['satisfy_money'] > $last_money_tt)throw new Exception('当前平台优惠券不满足使用条件');
                ...
                foreach($coupon_data as $k => &$v){  //判断每个店铺的均摊
                    ...
                    if($coupon_info['coupon_money'] >= $last_money_tt){
                        ...
                        if($num2 >= count($coupon_data)){
                            ...
                            $v['data']['pay_money'] = $price_tt - $v['data']['coupon_money'] + $v['data']['total_freight'];  //加上运费的金额
                        }else{
            ...
            if(round($pay_money_tt,2) != $pay_money){
...
//            throw new Exception('恭喜');
            ...
            switch($pay_type){
                ...
                case 2:
                    ...
                    $notify_url = SERVICE_FX."/user_v2/wx_pay/goods_wxpay_notify";
//                    $wxPay = new WxPay();
                $wxPay = new \app\user_v2\controller\WxPay();
