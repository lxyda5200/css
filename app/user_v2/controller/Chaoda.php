    ...
    public function chaodaList(){
        try{
            ...
            if(isset($store_id)&&!empty($store_id)){
                ...
                $total=Db::name('chaoda')->where('store_id',$store_id)->where('is_delete',0)->count();
            ...
            //首页潮搭
                ...
                $page = $this->request->has('page') ? $this->request->param('page') : 1 ;
                $size = $this->request->has('size') ? $this->request->param('size') : 30 ;
                $id="%".$id."%";
                $ids="'".$id."'";

                //非首页数据
                //统计数据
                $total=Db::name('chaoda')->where(("store_id > 0 AND is_delete = 0 AND category_id like $ids "))->count();
                $list = Db::view('chaoda','id,store_id,cover,description,title,category_id,fb_user_id,address,is_pt_user,is_delete,type,is_group')
                    ->view('store','cover as store_logo,store_name,address,lng,lat','chaoda.store_id = store.id','left')
...
//                if($id==9){//暂时默认9为探店  //如果是首页 则获取普通用户发布数据和探店数据
//                    //统计数据
//                    $id="'%".$id."%'";
//                    $total=Db::name('chaoda')->where(("is_delete = 0 AND (category_id like $id OR is_pt_user = 1) "))->count();
//                    $list = Db::view('chaoda','id,store_id,cover,description,title,category_id,fb_user_id,address,is_pt_user,is_delete,type,is_group')
//                        ->view('store','cover as store_logo,store_name,address','chaoda.store_id = store.id','left')
//                        ->view('user','avatar,nickname','chaoda.fb_user_id = user.user_id','left')
//                        ->where(("chaoda.is_delete = 0 AND (chaoda.category_id like $id OR chaoda.is_pt_user = 1) "))
//                        ->page($page,$size)
//                        ->order('chaoda.id','desc')
//                        ->select();
//
...
//            }
    ...
    public function chaodaDetail(){
        try{
            ...
            if($chaoda['is_delete']==1){
                return \json(self::callback(0,'该潮搭已删除！'));
            }
...
//统计最新
    ...
    public function chaodaCollectionList(){
        try{
            ...
            $user_id = $userInfo['user_id'];
            $total =  Db::name('chaoda_collection')->where('user_id',$user_id)->count();
            //统计
            if($total==0){
                $data['total'] = 0;
                $data['max_page'] = 0;
